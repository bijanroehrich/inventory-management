...
export default {
  ...
  setup() {
    ...
    // Derived summary stats as computed properties — not manually managed state
    ...
    // maxRevenue as computed so getBarHeight doesn't recalculate it on every bar render
    ...
    const formatMonth = (monthStr) => {
      // Parse YYYY-MM, map to localized month abbreviation
      const parts = monthStr.split('-')
      const year = parts[0]
      const monthIndex = parseInt(parts[1], 10) - 1 // 0-based
      const keys = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec']
      return t('months.' + keys[monthIndex]) + ' ' + year
    }

    const getBarHeight = (revenue) => {
      // maxRevenue is a precomputed computed property to avoid per-bar recalculation
